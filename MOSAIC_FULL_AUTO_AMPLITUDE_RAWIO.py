...
for NAME,FOLDER,IDX in zip(NAMES,FOLDERS,range(len(NAMES))):
    ...
    for scanspot in range(files.shape[0]):  
        ...
        if IDX == 6 or IDX == 7:  #Ces deux putains de manips a t=200ms...
            ...
            win_baseline_end = np.where(time>=0.2)[0][0]        
            
            window_begin = np.where(time>=0.2)[0][0]
            window_end = np.where(time>=0.5)[0][0]
            
            noise_begin = np.where(time>=0.6)[0][0]
            noise_end = np.where(time>=0.8)[0][0]
            
        else:
            
            noise_baseline_begin = np.where(time>=0.20)[0][0]
            noise_baseline_end = np.where(time>=0.21)[0][0]
            
            win_baseline_begin = np.where(time>=0.49)[0][0]
            win_baseline_end = np.where(time>=0.5)[0][0]        
