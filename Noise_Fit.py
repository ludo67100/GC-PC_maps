...
if __name__ == '__main__' :
    ...
    path = 'U:/01_ANALYSIS/data Theo inhibition/11-04-2019_DATAS.xlsx'
    ...
    a = fit_noise(noisemap,bins=50,polarity='inhbition',method='bimodal',closefig=False)
    
    
    ampl_in = np.ravel(pd.read_excel(path,sheet_name='Amp_I').values)
    from matplotlib import pyplot as plt
    plt.figure()
    n,bins,patches, = plt.hist(ampl_in, bins=50,density=0,facecolor='gray',alpha=0.5, label='noise distrubution') #Histogram
